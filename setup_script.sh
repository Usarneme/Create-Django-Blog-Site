...
then
  ...
  echo "10. Updating Installed Apps in mysite/settings.py"
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then # Linux
    sed -i "s/'django.contrib.staticfiles',/'django.contrib.staticfiles','blog.apps.BlogConfig',/g" mysite/settings.py
  elif [[ "$OSTYPE" == "darwin"* ]]; then # Mac OSX
    sed -i '' "s/'django.contrib.staticfiles',/'django.contrib.staticfiles','blog.apps.BlogConfig',/g" mysite/settings.py
  fi
  echo "10. Done"
...
def post_edit(request, pk):
  ...
  echo
  echo "To start the project, run ~ python manage.py runserver"
